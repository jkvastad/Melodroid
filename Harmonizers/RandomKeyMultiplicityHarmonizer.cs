    ...
    public class RandomKeyMultiplicityHarmonizer() : IMeasureHarmonizer
    {
        ...
        public List<Measure> MeasuresFromVelocities(List<int?[]> velocityMeasures)
        {
            ...
            foreach (var velocityMeasure in velocityMeasures)
            {
                ...
                //Select fundamental for scale from chord key multiplicity
                CurrentScaleFundamental = (CurrentScaleFundamental + keyMultiplicity[0].TakeRandom()) % 12;
                //Chord fundamental is the transpose of the scale fundamental
                // - if the scale root is 11 steps to the right of the chord root, then the chord root is one step to the right of the scale root
                CurrentChordFundamental = (12 - CurrentScaleFundamental) % 12;
