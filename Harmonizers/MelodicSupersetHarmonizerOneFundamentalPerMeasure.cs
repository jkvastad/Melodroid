...
{
    public class MelodicSupersetHarmonizerOneFundamentalPerMeasure(int[] chord) : IMeasureHarmonizer
    {
        ...
        public List<Measure> MeasuresFromVelocities(List<int?[]> velocityMeasures)
        {
            ...
            foreach (var velocityMeasure in velocityMeasures)
            {
                ...
                int melodyRoot = properKeysPerFundamental.Keys.OrderByDescending(key => properKeysPerFundamental[key].Count).First(); //select a fundamental and stick with it through the measure
